  ...
  >
    <v-list two-line class="carlist grey r_C lighten-4" light>
      <v-list-item>
        ...
        <v-list-item-content>
          <v-list-item-title>Prossimi appuntamenti</v-list-item-title>
    ...
    >
      <v-carousel-item v-for="event in Fevents" :key="event.pk" class="anccarouselitem mx-auto">
        <v-sheet>
          <!-- class="fill-height" -->
        ...
        <!-- intestazione evento -->
        <v-list two-line class="eventrif" :class="clFevent" light>
...
export default {
  ...
  props: {
    /*TEST 2020 07 29 DELEGA A STORE*/
    /*
    Fevents: {
      type: Array
    },
  ...
  methods: {
    FutureClearData() {
      this.$store.dispatch("getEventsFutureClearData");
    },
    getEventsFuture() {
  ...
  created() {
    console.log("Car-- call to FutureClearData");
    this.FutureClearData();
    console.log("Car-- call to getEventsFuture");
